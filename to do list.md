...
| # | Task | Status | Notes | Priority | Implementation Notes |
|---|-----|--------|-------|----------|---------------------|
| 1 | Show Face filter section when all 4 topics are completed | ⏳ Pending | Unlock face filter after completion | 🔴 Critical | **Requirement**: Face filter section becomes available only after all 4 topics completed. **Implementation**: Check completion status and show/hide face filter section. **Integration**: Connect to progress tracking system. **Dependency**: Must complete before face filter creation. |
| 2 | Cannot run through topic animation again, on 2nd run through, state will not change from scanning. Poster scans but no start button is seen, just go back to menu | ⏳ Pending | Fix animation replay issue | 🔴 Critical | **Requirement**: Allow multiple runs through same topic animation. **Issue**: Second run gets stuck in scanning state. **Implementation**: Reset state properly between runs. **Fix**: Ensure state transitions work on repeat visits. |
| 3 | Add UI to explain how to use this app | ⏳ Pending | Create help/instructions interface | 🔴 High | **Requirement**: User-friendly interface explaining app usage. **Implementation**: Modal or dedicated section with step-by-step instructions. **Content**: How to scan posters, navigate through topics, complete learning flow. **UX**: Clear, visual instructions for first-time users. |
| 4 | Add congratulatory animation for completing the topics and earning the badge. Redo congratulation message | ⏳ Pending | Completion celebration | 🔴 High | **Requirement**: Special animation and message for topic completion. **Implementation**: Animated celebration with badge earning. **Design**: Engaging completion experience. **UX**: Rewarding finish to learning journey. |
| 5 | On detection, animate the loading panel with text and particles | ⏳ Pending | Enhanced loading animation | 🔴 High | **Requirement**: Animated loading panel during poster detection. **Implementation**: Text animation and particle effects. **Design**: Engaging loading experience. **UX**: Visual feedback during detection process. |
| 6 | Dynamically add the profile pictures to the image elements in animation | ⏳ Pending | Dynamic profile images | 🔴 High | **Requirement**: Load profile pictures dynamically in animations. **Implementation**: Replace static images with dynamic loading. **Content**: Use actual profile images from assets. **Integration**: Connect to asset management system. |
| 7 | Animation topic 1 is dragging at the end | ⏳ Pending | Fix animation performance | 🔴 High | **Requirement**: Smooth animation performance for topic 1. **Issue**: Animation becomes slow/dragging at the end. **Implementation**: Optimize animation timing and performance. **Fix**: Ensure smooth playback throughout entire animation. |
| 8 | Complete animation content for topic 2 (Digital Marketing) | ⏳ Pending | Create topic-specific animation content | 🔴 High | **Requirement**: Replace placeholder content in timeline-topic-2.js with actual Digital Marketing themed content. **Implementation**: Adapt animation timeline with marketing-specific scenarios, characters, and educational content. **Content**: Digital marketing concepts, social media safety, online advertising awareness. **Integration**: Connect to existing animation system and AR scene manager. |
| 9 | Complete animation content for topic 3 (Data Science) | ⏳ Pending | Create topic-specific animation content | 🔴 High | **Requirement**: Replace placeholder content in timeline-topic-3.js with actual Data Science themed content. **Implementation**: Adapt animation timeline with data science-specific scenarios, characters, and educational content. **Content**: Data privacy, information security, responsible data handling. **Integration**: Connect to existing animation system and AR scene manager. |
| 10 | Complete animation content for topic 4 (Cybersecurity) | ⏳ Pending | Create topic-specific animation content | 🔴 High | **Requirement**: Replace placeholder content in timeline-topic-4.js with actual Cybersecurity themed content. **Implementation**: Adapt animation timeline with cybersecurity-specific scenarios, characters, and educational content. **Content**: Online safety, password security, recognizing cyber threats. **Integration**: Connect to existing animation system and AR scene manager. |
| 11 | Implement request camera access properly using navigator.mediaDevices.getUserMedia() if user initially denies access | ⏳ Pending | Replace fake permission system with real camera access | 🔴 High | **Current Issue**: Removed fake camera permission code that was just UI theater. **Requirement**: Implement proper camera permission handling using `navigator.mediaDevices.getUserMedia()` with retry logic for denied permissions. **Integration**: Should work with existing AR Scene Manager and MindAR system. **User Experience**: Provide clear feedback and retry options when camera access is denied. |
| 12 | Store campus value from UI and make it available during the topic 1: animation | ⏳ Pending | Pass campus selection to animation system | 🟡 Medium | **Requirement**: Campus value selected in UI should be accessible during topic 1 animation. **Implementation**: Store campus value globally and pass to animation timeline. **Integration**: Connect campus selection to animation data. |
| 13 | Create face filter | ⏳ Pending | Implement face filter functionality | 🟡 Medium | **Requirement**: Face filter feature for completed users. **Implementation**: Face detection and filter application. **Dependency**: Blocked until face filter section is shown (task 3). **Technology**: WebRTC face detection or similar. |
| 14 | Add a "help" button that shows the UI we have created from (2.) | ⏳ Pending | Help button integration | 🟡 Medium | **Requirement**: Help button that displays instructions UI. **Implementation**: Button that triggers help modal/section. **Integration**: Connect to help UI created in task 2. **UX**: Accessible from main menu or throughout app. |
| 15 | Redo progress tracking UI to be a scrollable carousel with image targets for the UI. When pressed, a slider with more information is shown | ⏳ Pending | Enhanced progress visualization | 🟡 Medium | **Requirement**: Interactive progress carousel with detailed information. **Implementation**: Replace current progress display with carousel. **Features**: Image targets, slider with details, interactive navigation. **UX**: More engaging than current simple progress bar. |
| 16 | All assets for animations appear at the start of an animation | ⏳ Pending | Fix asset visibility timing | 🟡 Medium | **Requirement**: Assets should appear at correct times during animation. **Implementation**: Control asset visibility with proper timing. **Issue**: All assets visible immediately instead of on schedule. **Fix**: Proper opacity/visibility control in timeline. |
| 17 | Must add StoryTribe cred - Website: please display "Created with StoryTribe" and link to our homepage "https://storytribeapp.com/" | ⏳ Pending | Add StoryTribe attribution | 🟡 Medium | **Requirement**: Display "Created with StoryTribe" with link to https://storytribeapp.com/. **Implementation**: Add attribution text and link. **Placement**: Footer or credits section. **Design**: Subtle but visible attribution. |
| 18 | Add proper styling to the quiz section - it's inconsistent - remove yellow from correct but not answered, add green tick / red cross icons? | ⏳ Pending | Improve quiz visual design | 🟡 Medium | **Requirement**: Consistent quiz styling with proper icons. **Implementation**: Remove yellow highlighting, add green/red icons. **Design**: Clear visual feedback for answers. **UX**: Intuitive answer indication. |
| 19 | Add a score to the quiz results and add more messages for if they got it right but also answers others wrong | ⏳ Pending | Enhanced quiz feedback | 🟡 Medium | **Requirement**: Detailed scoring and feedback system. **Implementation**: Score display and nuanced feedback messages. **Content**: Different messages for various answer combinations. **UX**: More informative quiz results. |
...
| 21 | Add a URL token that turns on the DEBUG UI eg: ?debug=true (show it) | ⏳ Pending | Debug mode via URL parameter | 🟢 Low | **Requirement**: URL parameter to enable debug UI. **Implementation**: Check for ?debug=true and show debug elements. **Features**: Console logs, state information, testing tools. **Integration**: Use existing URL parameter system. |
| 22 | UI buttons can be double clicked causing timing issues in the animations | ⏳ Pending | Prevent double-click issues | 🟢 Low | **Requirement**: Fix timing issues from double-clicking buttons. **Implementation**: Add click debouncing or disable buttons during animations. **Prevention**: Prevent rapid successive clicks that break animation flow. **UX**: Smooth, reliable button interactions. |
| 23 | Add a visible, spatial loading / Ready card before the start button is pressed | ⏳ Pending | Loading state indicator | 🟢 Low | **Requirement**: Visual loading indicator before start button appears. **Implementation**: Loading card with spatial positioning. **UX**: Clear feedback that system is preparing. **Design**: Consistent with app's visual language. |
| 24 | HTML <a> tags are unstyled | ⏳ Pending | Style anchor tags | 🟢 Low | **Requirement**: Consistent styling for all links. **Implementation**: Add CSS for <a> tags. **Design**: Match app's visual theme. **UX**: Clear indication of clickable links. |
| 25 | Add a credits section? | ⏳ Pending | Create credits/attributions section | 🟢 Low | **Requirement**: Dedicated section for credits and attributions. **Implementation**: New section with team credits, libraries used, etc. **Content**: Development team, third-party libraries, assets. **Design**: Professional credits layout. |
| 26 | Assign colours to each of the 4 topics and filter background colours, and assign specific images on the hero sections for Video, quiz and summary | ⏳ Pending | Topic-specific theming | 🟢 Low | **Requirement**: Unique colors and images for each topic. **Implementation**: Color scheme and hero images per topic. **Design**: Consistent theming throughout topic flow. **UX**: Visual differentiation between topics. |
| 27 | Add a general menu back button that goes back to the menu state and resets topics and html | ⏳ Pending | Global back to menu functionality | 🟢 Low | **Requirement**: Back button available throughout app. **Implementation**: Reset state and return to menu. **Functionality**: Clear current topic, reset UI state. **UX**: Easy navigation back to main menu. |
| 28 | Remove go back button from summary | ⏳ Pending | Clean up summary UI | 🟢 Low | **Requirement**: Remove unnecessary back button from summary. **Implementation**: Remove button from summary section. **UX**: Cleaner summary interface. **Navigation**: Users should continue forward, not go back. |
