...
/***/ (function(module, exports) {
	...
	// Modern Web Components v1 registration
	if (window.customElements) {
		window.customElements.define('a-timeline', class extends HTMLElement {});
		window.customElements.define('a-timeline-group', class extends HTMLElement {});
		window.customElements.define('a-timeline-animation', class extends HTMLElement {});
	} else {
		// Fallback for older browsers (though this component likely won't work in them anyway)
	...
	AFRAME.registerComponent('animation-timeline', {
	  ...
	  addAnimationToTimeline: function (animationEl, offset) {
	    ...
	    for (i = 0; i < els.length; i++) {
	      // Fix: Look for animation in the correct A-Frame component structure
	      var entity = els[i];
	      var animationComponent = entity.components.animation;
	      
	      if (!animationComponent) {
	        throw new Error('Could not find animation component for `' +
	                        animationEl.getAttribute('select') + '`.');
	      }
	      
	      // Extract the animation name without the 'animation__' prefix
	      var animName = animationEl.getAttribute('name');
	      var animation = animationComponent.animations[animName];
	      
	      if (!animation) {
	        throw new Error('Could not find animation `' + animName + '` for `' +
	                        animationEl.getAttribute('select') + '`.');
	      }
	      
	      // Create config from the animation
	      config = {
	        target: entity,
